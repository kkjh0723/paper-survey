...
### ★ 작성 원칙: 논문을 안 읽은 사람이 읽고 이해할 수 있어야 한다
독자는 이 분야를 대략 아는 연구자이지만 **이 논문은 처음 본다**. 아래를 반드시 지킨다.

1. **모든 전문용어는 첫 등장에서 한 번 풀어쓴다.** 괄호로 짧게 붙이는 방식.
   - 나쁨: "flow matching action head로 액션 청크를 생성"
   - 좋음: "flow matching(무작위 노이즈에서 정답 궤적 쪽으로 흐르는 방향을 학습해, 몇 스텝 만에 액션을 만들어내는 생성 방식) 헤드로 액션 청크를 생성"
   - 대상 예: flow matching, InfoNCE, beam search, global pooling, latent/잠재 토큰, causal mask,
     action chunk, cross-embodiment, offline RL, advantage, distillation, OOD, ablation 등.
2. **논문이 만든 고유 명칭(방법·데이터셋·설정·실험군 이름)은 "그게 실제로 뭔지"를 구체적으로 쓴다.**
   이름만 반복하는 것은 금지. 특히 **결론의 근거가 되는 설정 이름**은 반드시 구체화한다.
   - 나쁨: "직교 설계(orthogonal design)로 학습하면 OOD 78.2%"
   - 좋음: "전체 64가지 조합을 다 모으는 대신, **어느 두 요인을 짝지어 봐도 모든 짝이 한 번씩은
     등장하도록** 16가지만 고른 학습 세트(orthogonal design)로 학습하면 OOD 78.2%"
3. **수식은 기호를 정의하지 않은 채 던지지 않는다.** 먼저 한국어로 무슨 뜻인지 말하고, 수식은 보조로.
   기호 하나하나(q, p, l, z, a 등)가 무엇을 가리키는지 밝힌다.
4. **숫자는 "무엇 대비 무엇인지" 맥락과 함께.** 단독 수치 금지.
   - 나쁨: "성공률 47.2%" → 좋음: "9개 실로봇 태스크 평균 성공률 47.2% (같은 태스크에서 π0.5는 29.4%)"
5. **약어는 처음에 풀어쓴다.** "OOD(out-of-distribution, 학습 때 못 본 상황)".
6. **각 Method 소제목마다 "왜 이렇게 했는지" 한 줄 직관**을 넣는다. 가능하면 `.callout`으로 비유·직관 제공.
   - 예: "직관: CLS 토큰 하나로 압축하면 '어디에' 정보가 사라지고, 전 프레임을 다 보게 하면 미래를
     훔쳐본다. block-causal은 그 사이의 절충이다."
7. **문장은 짧게.** 한 문장에 새 개념 2개 이상 넣지 않는다.
